...
float4 BColor :  COLOR <String uiname="Border Color";>  = {0, 0, 0, 1};
//texture
...
float4x4 tTexDsiplace : TEXTUREMATRIX <string uiname="Displace Texture Transform";>;
     ...
     float4 TexCddispla : TEXCOORD1)
{
    ...
    lookup = mul(TexCd, tTexDsiplace);
    lookup.w = 0;
    float4 tx = tex2Dlod(SampDisplace,lookup);
    ...
    Out.TexCddispla = mul(TexCd, tTexDsiplace);
...
{   float texelSize =  1.0f / texSize ;
    float tl = abs(tex2D (SampDisplace, In.TexCddispla +texelSize  * float2(-1, -1)).x);   // top left
    float  l = abs(tex2D (SampDisplace, In.TexCddispla +texelSize  * float2(-1,  0)).x);   // left
    float bl = abs(tex2D (SampDisplace, In.TexCddispla +texelSize  * float2(-1,  1)).x);   // bottom left
    float  t = abs(tex2D (SampDisplace, In.TexCddispla +texelSize  * float2( 0, -1)).x);   // top
    float  b = abs(tex2D (SampDisplace, In.TexCddispla +texelSize  * float2( 0,  1)).x);   // bottom
    float tr = abs(tex2D (SampDisplace, In.TexCddispla +texelSize  * float2( 1, -1)).x);   // top right
    float  r = abs(tex2D (SampDisplace, In.TexCddispla +texelSize * float2( 1,  0)).x);   // right
    float br = abs(tex2D (SampDisplace, In.TexCddispla +texelSize  * float2( 1,  1)).x);   // bottom right
